...
def multi_model_mode(model_filenames):
    ...
    for i in range(1, len(models) + 1) if args.permutate else [1]:
        for comb in itertools.combinations(range(len(models)), i):
            ...
            if best_true_acc < true_acc:
                best_true_acc = true_acc
                best_true_acc_comb = comb
            
            if best_false_acc < false_acc:
                best_false_acc = false_acc
                best_false_acc_comb = comb

            if best_det_acc < det_acc:
                best_det_acc = det_acc
                best_det_comb = comb

    print("\n\n\n==================\nBest accuracy combination")
    evaluate_combination(best_acc_comb, burp_results, not_burp_results, model_filenames)

    print("\n\n\n==================\nLeast false negatives combination")
    evaluate_combination(best_true_acc_comb, burp_results, not_burp_results, model_filenames)

    print("\n\n\n==================\nLeast false positives combination")
    evaluate_combination(best_false_acc_comb, burp_results, not_burp_results, model_filenames)

    print("\n\n\n==================\nBest detections accuracy combination")
    evaluate_combination(best_det_comb, burp_results, not_burp_results, model_filenames)
